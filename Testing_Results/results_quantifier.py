...
def display(img, costmap, costmap_by_hand, rectangle_list, grid_list, max_cost, min_cost) :
    """
    ...
    for x in range(viz.X) :
        for y in range(viz.Y) :
            ...
            if costmap_by_hand[y,x]!= 0 :
                ...
                #Normalizing the content
                value = np.uint8(((costmap_by_hand[y,x]-min_cost)/(max_cost-min_cost))*255)
    ...
    costmapviz_hand = cv2.applyColorMap(src=costmapviz_hand, colormap=cv2.COLORMAP_JET)
    costmapviz_diff = cv2.applyColorMap(src=costmapviz_diff, colormap=cv2.COLORMAP_JET)

    #Displaying the results
    imgviz = cv2.resize(imgviz, (IMAGE_W//2, IMAGE_H//2))
    costmapviz = cv2.resize(cv2.flip(costmapviz, 0),(IMAGE_W//2, IMAGE_H//2))
    result = np.vstack((imgviz,costmapviz))

    costmapviz_hand = cv2.resize(cv2.flip(costmapviz_hand, 0), (IMAGE_W//2, IMAGE_H//2))
    costmapviz_diff = cv2.resize(cv2.flip(costmapviz_diff, 0), (IMAGE_W//2, IMAGE_H//2))
